...
class AuthManager {
    ...
    async login(userId = null) {
        ...
        // Normalise full_name so UI always has something to display
        if (!user.full_name) {
            user.full_name = user.display_name || user.name ||
    ...
    saveSession() {
        if (this.currentUser) {
            ...
            // Always keep 'currentUser' key consistent — used as fallback by auth.js
            const userId = this.currentUser.profile_id || this.currentUser.uid;
            localStorage.setItem('currentUser', userId);
            if (this.currentUser.uid !== 'guest' && window.mizanoStorage) {
                window.mizanoStorage.setCurrentUser(userId);
            }
        }
    }
    ...
    /**
     * isLoggedIn — async check used by index.html splash screen.
     * Returns true if a real (non-guest) session exists in localStorage.
     */
    async isLoggedIn() {
        // If already in memory, use that
        if (this.currentUser && this.currentUser.uid !== 'guest') return true;

        // Check IndexedDB / localStorage session
        const saved = localStorage.getItem(this.sessionKey);
        if (saved) {
            try {
                const parsed = JSON.parse(saved);
                if (parsed && parsed.uid && parsed.uid !== 'guest') {
...
/**
 * window.MizanoAuth — compatibility shim for index.html splash screen
 * and any legacy code referencing this namespace.
 */
window.MizanoAuth = {
    isLoggedIn: () => window.authManager.isLoggedIn(),
    login: (userId) => window.authManager.login(userId),
